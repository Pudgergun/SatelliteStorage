...
namespace SatelliteStorage.UI
{
    ...
    public class UIDynamicItemCollection : UIElement
    {
        ...
        public Dictionary<int, IDriveItem> _driveItems { get; private set; } = new Dictionary<int, IDriveItem>();
        private Dictionary<int, IDriveItem> _autoImportItems = new Dictionary<int, IDriveItem>();
        private int _itemsPerLine;

        private List<SnapPoint> _dummySnapPoints = new List<SnapPoint>();

        public int hoverItemIndex { get; private set; } = -1;

        private bool showAutoImport;
        ...
        public void SetDisplayAutoImport(bool toggle) => showAutoImport = toggle;
        ...
        protected override void DrawSelf(SpriteBatch spriteBatch)
        {
            ...
            for (int i = startItemIndex; i < endItemIndex; i++)
            {
                ...
                IDriveItem driveItem = _driveItems.ContainsKey(i) ? _driveItems[i] : new DriveItem();
                ...
                if (showAutoImport)
                {
                    if (_autoImportItems.ContainsKey(inv.type))
                        inv.favorited = true;
                    else
                        inv.favorited = false;
                }
                ...
                if (TextureAssets.Item[num2].State == 0)
                {
                    num--;
                }
            ...
            while (_itemIdsToLoadTexturesFor.Count > 0 && num > 0)
            {
                ...
                if (TextureAssets.Item[num3].State == 0)
                {
        ...
        private void GetGridParameters(out int startX, out int startY, out int startItemIndex, out int endItemIndex)
        {
            ...
            int num2 = (int)Math.Ceiling(viewCullingArea.Height / 44f) * _itemsPerLine;
        ...
        {
            public List<IDriveItem> items = new List<IDriveItem>();
        ...
        public void SetContentsToShow(
            List<int> itemIdsToShow,
            List<IDriveItem> driveItems,
            Dictionary<int, IDriveItem> autoImportItems = null
        )
        {
        ...
        public void UpdateSize()
        {
            ...
            int num2 = (int)Math.Ceiling(_itemIdsAvailableToShow.Count / (float)num);
